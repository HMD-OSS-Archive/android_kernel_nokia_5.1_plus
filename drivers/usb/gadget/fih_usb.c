...
static void fih_switch_all_port(void)
{
	...
	if (android_usb_product_id() != 0xC026)
	{
		sw_pid.state = -1;
		fihusb_set_state(&sw_pid, 0xC026);
		pr_info("%s : change PID to 0xC026\n", __func__);
	}else {
		pr_info("%s : already change PID to 0xC026\n", __func__);
	}
}
...
static void usb_all_port_setting(struct work_struct *work)
{
	...
	if ((android_usb_product_id() != 0xC026) && (check_Allport_num<MAX_CHECK_NUM))
	{
	       if(selinux_enforcing != 0) {
